...
train_inte = pd.read_csv('/Users/majiefeng/Desktop/研究生课程/数据挖掘/homework_jfma/个贷违约预测/code/baseline_user_0.891/raw_data/train_internet.csv')
...
def train_model(data_, test_, y_, folds_):
    ...
    return oof_preds, test_[['loan_id', 'is_default']], feature_importance_df
...
train_data['earlies_credit_mon'] = pd.to_datetime(train_data['earlies_credit_mon'].map(findDig), dayfirst = True)
test_public['earlies_credit_mon'] = pd.to_datetime(test_public['earlies_credit_mon'].map(findDig), dayfirst = True)
...
# 暂时不变
# train_inte = train_inte.rename(columns={'is_default':'is_default'})
...
roc_auc_score(IntePre['isDef'],IntePre.is_default)
## 选择阈值0.05，从internet表中提取预测小于该概率的样本，并对不同来源的样本赋予来源值
InteId = IntePre.loc[IntePre.is_default<0.05, 'loan_id'].tolist()
...
sns.distplot(IntePre['is_default'],color="black", kde=True,bins=120, label='train_data')
...
test_preds.rename({'loan_id': 'id'}, axis=1)[['id', 'is_default']].to_csv('nn2.csv', index=False)
...
import pandas as pd
train_data = pd.read_csv('/Users/majiefeng/Desktop/研究生课程/数据挖掘/homework_jfma/个贷违约预测/code/baseline_user_0.891/raw_data/train_public.csv')
test_data = pd.read_csv('/Users/majiefeng/Desktop/研究生课程/数据挖掘/homework_jfma/个贷违约预测/code/baseline_user_0.891/raw_data/test_public.csv')
sub=pd.read_csv("nn2.csv")
sub=sub.rename(columns={'id': 'loan_id'})
sub.loc[sub['is_default']<0.5,'is_default'] = 0
nw_sub=sub[(sub['is_default']==0)]
...
nw_train_data.to_csv("/Users/majiefeng/Desktop/研究生课程/数据挖掘/homework_jfma/个贷违约预测/code/baseline_user_0.891/raw_data/nw_train_public.csv",index=0)
...
from sklearn.model_selection import StratifiedKFold
from dateutil.relativedelta import relativedelta
train_data = pd.read_csv('/Users/majiefeng/Desktop/研究生课程/数据挖掘/homework_jfma/个贷违约预测/code/baseline_user_0.891/raw_data/nw_train_public.csv')
submit_example = pd.read_csv('/Users/majiefeng/Desktop/研究生课程/数据挖掘/homework_jfma/个贷违约预测/code/baseline_user_0.891/raw_data/submit_example.csv')
test_public = pd.read_csv('/Users/majiefeng/Desktop/研究生课程/数据挖掘/homework_jfma/个贷违约预测/code/baseline_user_0.891/raw_data/test_public.csv')
train_inte = pd.read_csv('/Users/majiefeng/Desktop/研究生课程/数据挖掘/homework_jfma/个贷违约预测/code/baseline_user_0.891/raw_data/train_internet.csv')
...
folds = KFold(n_splits=5, shuffle=True, random_state=546789)
oof_preds, test_preds, importances = train_model(train, test, y, folds)
test_preds.rename({'loan_id': 'id'}, axis=1)[['id', 'is_default']].to_csv('baseline891.csv', index=False)